...
command! -nargs=0 OR   :call     CocActionAsync('runCommand', 'editor.action.organizeImport')
        ...
        vim.keymap.set('n', '<leader>ds', ':Telescope coc document_symbols<CR>', {
            desc = '[D]ocument [S]ymbols'
        })
        vim.keymap.set('n', '<leader>ws', ':Telescope coc workspace_symbols<CR>', {
            desc = '[W]orkspace [S]ymbols'
        })
        vim.keymap.set('x', '<leader>ca', '<Plug>(coc-codeaction-selected)<CR>', {
            desc = '[C]ode [A]ction'
        })
